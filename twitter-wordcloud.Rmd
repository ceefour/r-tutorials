...
# tl_dakwatuna <- userTimeline('dakwatuna', n=500)
# write.csv(twListToDF(tl_dakwatuna), '~/git/r-tutorials/tl_dakwatuna_2015-04-02.csv')
df <- read.csv('tl_dakwatuna_2015-04-02.csv')
...
# tl_suaradotcom <- userTimeline('suaradotcom', n=500)
...
# tl_kompascom <- userTimeline('kompascom', n=500)
...
# tl_vivanews <- userTimeline('VIVAnews', n=500)
...
tdm <- TermDocumentMatrix(corpus,
  control = list(stripWhitespace = TRUE, tolower = TRUE,
                 removeNumbers = TRUE,
                 removePunctuation = TRUE,
                 stopwords = c(stopwords_id, 'viva', 'vivanews', 'vivacoid') ))
m <- as.matrix(tdm)
wf <- sort(rowSums(m), decreasing=TRUE)
wf <- wf[wf > 0]
dm <- data.frame(word=names(wf), freq=wf)
...
# tl_liputan6dotcom <- userTimeline('liputan6dotcom', n=500)
...
# tl_pkspiyungan <- userTimeline('pkspiyungan', n=500)
...
# tl_mtlovenhoney <- userTimeline('MTlovenhoney', n=500)
# write.csv(twListToDF(tl_mtlovenhoney), '~/git/r-tutorials/tl_mtlovenhoney_2015-04-02.csv')
df <- read.csv('tl_mtlovenhoney_2015-04-02.csv')
corpus <- Corpus(VectorSource(df$text))
tdm <- TermDocumentMatrix(corpus,
  control = list(stripWhitespace = TRUE, tolower = TRUE,
                 removeNumbers = TRUE,
                 removePunctuation = TRUE,
                 stopwords = c(stopwords_id, 'mtlovenhoney', 'mario', 'teguh') ))
m <- as.matrix(tdm)
wf <- sort(rowSums(m), decreasing=TRUE)
wf <- wf[wf > 0]
dm <- data.frame(word=names(wf), freq=wf)
...
# tl_farhatabbaslaw <- userTimeline('farhatabbaslaw', n=500)
# write.csv(twListToDF(tl_farhatabbaslaw), '~/git/r-tutorials/tl_farhatabbaslaw_2015-04-02.csv')
df <- read.csv('tl_farhatabbaslaw_2015-04-02.csv')
corpus <- Corpus(VectorSource(df$text))
tdm <- TermDocumentMatrix(corpus,
  control = list(stripWhitespace = TRUE, tolower = TRUE,
                 removeNumbers = TRUE,
